...
//function to delete a node in the linked list containing the data value num
//removes the all occurrences of the number from the list
void delete_node(int num)
{
	//intializing the curent pointer and previous pointer
	struct node *current_pointer = phead;
	struct node *prev_pointer = NULL;
	//looping through the linked list to find the number and delete it
	while (current_pointer != NULL)
	{
		//checks if the current nodes number is the number we want to delete
		if (current_pointer->datavalue == num)
		{
			//checks if the current pointer is the head pointer
			if (phead->datavalue == num)
			{
				//updates the head pointer
				phead = current_pointer->pnextnode;
				//frees the pointer
				free(current_pointer);
				//re-sets the value of the current pointer so the list can continue to be iterated through to check for multiple occurrences
				current_pointer = phead;
				continue;
			}
			//checks if the current pointer is the back pointer
			if (current_pointer->pnextnode == NULL)
			{
				//updates the back pointer
				pback = prev_pointer;
				//sets back pointers next node back to null
				pback->pnextnode = NULL;
				//frees pointer
				free(current_pointer);
				break;
			}
			//removes the current node from the list
			prev_pointer->pnextnode = current_pointer->pnextnode;
			//frees current pointer
			free(current_pointer);
			//re-sets the value of the current pointer so the list can continue to be iterated through to check for multiple occurrences
			current_pointer = prev_pointer->pnextnode;
			continue;
		}
		//updates previous and current pointer for next loop iteration if the desired node was not found and deleted
		prev_pointer = current_pointer;
		current_pointer = current_pointer->pnextnode;
	}
}
